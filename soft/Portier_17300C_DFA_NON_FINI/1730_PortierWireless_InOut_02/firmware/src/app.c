...
void CallBackTimer2(APP_STATES newState)
{
    appData.state = newState;
    if(APP_GetIsDoor())
    {
        DoDebounce(&swRing, !PORTBbits.RB1);        
    } else
    {
        DoDebounce(&swEnter, PORTBbits.RB7);
        DoDebounce(&swWait,  PORTBbits.RB8);
        DoDebounce(&swBusy,  PORTBbits.RB9);       
    }
}
...
void Door_Mgmt(void)
{
    ...
    static uint16_t waitStartTimeout = TIME_WAIT_TIMEOUT;
    ...
    //Reset le debounce pendant 10 tours
    CheckDebounce();
    
    //bouton appuyé ?
    CheckStartRing(&doorState);
                ...
                {                
                    ...
                    {                       
                        ...
                        if (cycleCntr%100==0)
                        {
                            ...
                            //sprintf((char*)msg, "[S%dD%02d%03d]",getSerialRF(), timeoutCntr/10, appData.batVoltage/10);
...
void Bell_Mgmt(void)
{
    #define BELL_TIMEOUT 10000 //en 1/100e de sec.
    ...
    switch(bellState)
    {
        ...
        case BELL_STATE_IDLE:
            ...
            {
                if (CheckSerialDoor(msg)) //message bien en provenance du bouton de porte ?
                {
                    timeoutCntr = 0;
                    cycleCntr = 0;
                    answerNr = -1;  //pas encore de réponse
                    sprintf((char *)answerFrame, "[B%dG]", GetActifSerialNbr()); //réponse : entrez !
                    RF_SendMessage(answerFrame, 0); //envoi réponse   
        ...
        case BELL_STATE_RING:
            ...
            {        
                ...
                Led_SetMode(LED_ID_BUSY, LED_MODE_OFF);
                answerNr =0;
                sprintf((char *)answerFrame, "[B%dW]", GetActifSerialNbr()); //réponse : entrez !
            }  
